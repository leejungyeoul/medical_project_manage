...
router.post('/', (req, res, next) => {
  ...
  if(m_typ == 'push'){
    //알림 전송
    if(req.body.is_Receiver == 'admin'){
      try {
        // Mysql Api 모듈(CRUD)
        var dbconnect_Module = require('./dbconnect_Module');
    
        //Mysql 쿼리 호출정보 입력
        req.body.mapper = 'MessageMapper';//mybatis xml 파일명
        req.body.crud = 'insert';//select, insert, update, delete 중에 입력
        req.body.mapper_id = 'insertNoticeAdminMessage';
      ...
      try {
        ...
        req.body.mapper = 'MessageMapper';//mybatis xml 파일명
        req.body.crud = 'insert';//select, insert, update, delete 중에 입력
        req.body.mapper_id = 'insertNoticeMessage';
    ...
    try {
      ...
      req.body.mapper_id = 'selectNoticeMessageList';
    ...
    try {
      ...
      req.body.mapper_id = 'updateNoticeMessageFlag';
    ...
    let transporter = nodemailer.createTransport({
      ...
      auth: {
        ...
        pass: '01032883398'          // gmail 계정의 비밀번호를 입력
      }
    });
